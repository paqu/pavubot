...
server_socket.on('message', function(msg, rinfo) {
        ...
        var rectThickness = 2;
        var id = msg.slice(0,27).toString();
        cv.readImage(msg.slice(27), function (err,im) {
            if (err) throw err;
            ...
            im.detectObject(faceCascade,{}, function(err, faces) {
                ...
                video_nsp.in(id).emit("user:robot:frame",{frame: im.toBuffer()});
            });
        });
});

server_socket.bind('8888',"0.0.0.0");
...
app.post('/api/wanted/', function (req,res) {
    ...
    var index    = getElementIndex(wantedList,id);
    var robot_id = findRobotIdByVideoId(robots, chanel);

    if (index != -1 && !isRecognized(wantedList, index)) {
        increment(wantedList,index);
        if (getRecognizedCount(wantedList, index) == 10) {
            setRecognizeStatus(wantedList, index, true);
            recognized_count++;
        }
        if (recognized_count == wantedList.length) {
            logger("[emit] user:robots_list:all_wanted");
...
function getRecognizedCount(arr, index) {
    return arr[index].recognized_count;
}

function increment(arr, index) {
    arr[index].recognized_count++;
}

function setRecognizeStatus(arr, index, val) {
   arr[index] = val;
}

function isRecognized(arr, index) {
    if (arr[index] != undefined)
        return arr[index].isRecognized;
}
...
function findRobotIdByVideoId(arr,id) {
    for (var i = 0; i < arr.length; i++) {
        if (arr[i].video_socket_id == id) {
            return arr[i].control_socket_id;
        }
    }
    return -1;
}
